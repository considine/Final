...
void pipes::movetime(){
	...
	srand(time(NULL));
		...
		newx = getx(i);
		setx(i, newx-speed); //iterate pipes oner pixel
	}
}
...
void pipes::speedup(){
	speed++;  //speed up the moving pipes
}

void pipes::slowdown() {
	speed--;  //slow the moving pioes
}

int pipes::getspeed() {
	return speed;  //to interact with graphics.cpp
}


void pipes::changegap(int num) {
	if (num==0) gap++;
	else gap--;
}
